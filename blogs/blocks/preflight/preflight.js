...
const tabs = signal([{ title: 'General', selected: true }, { title: 'SEO' }]);
...
function Preflight() {
  return html`
    <div class="preflight-heading">
      ...
      >
        ${tabs.value.map(
          (tab, idx) => html`<${TabButton} tab=${tab} idx=${idx} />`
        )}
      </div>
    </div>
    <div class="preflight-content">
      ${tabs.value.map(
        (tab, idx) => html`<${TabPanel} tab=${tab} idx=${idx} />`
      )}
    </div>
  `;
}
...
function preloadAssets(el) {
  return new Promise((resolve) => {
    ...
    bg.addEventListener('load', () => {
      ...
      const check = createTag('link', {
        rel: 'preload',
        as: 'image',
        href: `${base}${IMG_PATH}/check.svg`,
      });
      const expand = createTag('link', {
        rel: 'preload',
        as: 'image',
        href: `${base}${IMG_PATH}/expand.svg`,
      });
